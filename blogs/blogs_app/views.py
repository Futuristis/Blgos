...
def new_entry(request, topic_id):
    ...
    if request.method!='POST':
        ...
        if form.is_valid():
            ...
            new_entry.save()
            return redirect('blogs_app:topic', topic_id=topic_id)
    
    # Display a blank or invalid form.
    context = {'topic':topic,'form':form}
    return render(request, 'blogs_app/new_entry.html',context)
...
def edit_entry(request, entry_id):
    ...
    if request.method !='POST':
        ...
        if form.is_valid():
            form.save()
            return redirect('blogs_app:topic',topic_id=topic.id)
    context = {'entry':entry, 'topic':topic, 'form':form}
    return render(request,'blogs_app/edit_entry.html',context)